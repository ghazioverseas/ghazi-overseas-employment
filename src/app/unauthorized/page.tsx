import Link from "next/link";
import { LockKeyhole, ArrowLeft } from "lucide-react";
import { Button } from "@/components/ui/button";

export default function UnauthorizedPage() {
  return (
    <div className="flex min-h-[calc(100vh-200px)] flex-col items-center justify-center p-4 text-center">
      <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-amber-100 text-amber-700 mb-6 shadow-sm">
        <LockKeyhole className="h-8 w-8" />
      </div>
      <h1 className="text-4xl font-extrabold tracking-tight text-slate-900 sm:text-5xl">401 - Authentication Required</h1>
      <p className="mt-3 max-w-md text-base text-slate-600">
        You must be signed in with a valid candidate or admin account to view this resource.
      </p>
      <div className="mt-8 flex gap-4">
        <Link href="/login">
          <Button size="lg" className="bg-blue-600 hover:bg-blue-700 gap-2">
            Sign In Now
          </Button>
        </Link>
        <Link href="/">
          <Button size="lg" variant="outline" className="gap-2">
            <ArrowLeft className="h-4 w-4" /> Home
          </Button>
        </Link>
      </div>
    </div>
  );
}
